...
#setwd("C:/Users/swapnajoshi/Documents/UCLA_research/IBSmicrobiomeProject/biopsy_scor_microbiome/")
setwd("C:/Users/swapnajoshi-admin/Documents/swapna/qiime_output")
...
sampleTree = flashClust(dist(datExpr0), method = "maximum")
...
clust1 = cutreeStatic(sampleTree, cutHeight = 0.6, minSize = 1)
table(clust1)
# clust1
...
####################################################### get the distances from qiime and construct a dendrogram

# library(FactoMineR)
# 
# unweiUnifracDist<-read.delim("unweighted_unifrac_pc_mod.txt", sep="", row.names=1)
# unweiUnifracDist1<-unweiUnifracDist[,1:3]
# unweiUnifracDist1<-apply(unweiUnifracDist1, 2, factor)
# 
# res.mca = MCA(unweiUnifracDist1, ncp=3, graph=FALSE, na.method="Average")
...
# dim(ma.phylum)
ma.species<-sd.species
dt.phylum<-as.data.frame(t(d.phylum))
dt.phylum$sd1<-apply(dt.phylum,1, sd)
hist(dt.phylum$sd1)
sd.phylum = subset(dt.phylum, dt.phylum$sd1>=0.02); dim(sd.phylum)
sd.phylum<-as.data.frame(sd.phylum)
sd.phylum$sd1<-NULL
ma.phylum<-sd.phylum
...
summary(lm(var.dat[,1]~ map2$Group))
...
p.traits1<-matrix(NA, nrow=16, ncol=1)
for ( i in 1: 16)
...
p.traits2<-matrix(NA, nrow=16, ncol=1)
for ( i in 1: 16)
...
p.traits3<-matrix(NA, nrow=16, ncol=1)
for ( i in 1: 16)
...
P<-ggplot(data3, aes(k__Bacteriap__Firmicutesc__Clostridiao__Clostridialesf__Lachnospiraceaeg__Coprococcuss__,hsa_miR_27b_3p_0))
P+geom_point(aes(color=Dx))
plot(data2$"k__Bacteria; p__Firmicutes; c__Clostridia; o__Clostridiales; f__Lachnospiraceae; g__Coprococcus; s__", data2$"hsa-miR-27b-3p|0")
