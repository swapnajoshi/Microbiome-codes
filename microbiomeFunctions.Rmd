---
title: "microBiomeFunctions"
author: "Swapna Mahurkar-Joshi"
date: "April 10, 2017"
output: html_document
---
##### Function to make a dendrogram,include colorbars for variables
```{r}
# Input distance matrix

dendColorBar <- function(x,y, clustMethod) {
  library(dendextend)
  library(flashClust)
  set.seed(2210)
  sampleTree = flashClust(x, method = clustMethod)
  dat.ordered <- x[sampleTree$order,]
  clust1 = cutreeStatic(sampleTree, cutHeight = 4, minSize = 1)
  lab1 <- y[sampleTree$labels,1]
  df2  <- as.data.frame(cbind(clust1,as.character(lab1)))
  print(table(df2[,1], df2[,2]))
  df2$colorDx <- gsub(levels(factor(df2[,1]))[1],"red",df2[,1])
  df2$colorDx <- gsub(levels(factor(df2[,1]))[2],"green",df2$colorDx)
  df2$colorBH <- gsub(levels(factor(df2[,2]))[1],"red",df2[,2])
  df2$colorBH <- gsub(levels(factor(df2[,2]))[2],"green",df2$colorBH)
  df2$colorBH <- gsub(levels(factor(df2[,2]))[3],"blue",df2$colorBH)
  df2$colorBH <- gsub(levels(factor(df2[,2]))[4],"pink",df2$colorBH)

  plot(sampleTree, main = "Heirachical Clustering", sub="", xlab="", 
  cex.lab = 1.5, cex.axis = 1.5, cex.main = 1.5, labels=lab1)
  abline(h = 4, col = "red")
  colored_bars(colors= df2[,c(3,4)], dend = sampleTree, y_shift = -1.0, 
               rowLabels = c("Dx", "Bowel Habit"),sort_by_labels_order 	
             = TRUE)
}

```

# Splitting Taxonomic data at semicolon
...
```{r}
# Function to summerize data at different taxonomic levels
